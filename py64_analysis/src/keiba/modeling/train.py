...
class DistanceBucketWinProbabilityModel:
    ...
    def _bucket_series(self, X: pd.DataFrame) -> pd.Series:
        if X is None or len(X) == 0 or "distance" not in X.columns:
            return pd.Series([None] * (0 if X is None else len(X)), index=getattr(X, "index", None), dtype=object)
    ...
    def _predict_p_model(self, X: pd.DataFrame, *, raw_score: bool = False) -> np.ndarray:
        ...
        for b in sorted(set(buckets.dropna().tolist())):
            ...
            booster = getattr(m, "lgb_model", None) if m is not None else getattr(self.fallback_model, "lgb_model", None)
            if booster is None:
                continue
    ...
    def predict(
        ...
        p_mkt_s = pd.to_numeric(pd.Series(p_mkt, index=X.index), errors="coerce").fillna(0.0).astype(float)
        buckets = self._bucket_series(X)
        cal = getattr(self.fallback_model, "calibrator", None)

        if return_residual_meta:
            b = buckets.iloc[0] if len(buckets) else None
            m = self.bucket_models.get(int(b)) if (b is not None and int(b) in self.bucket_models) else self.fallback_model
            p_raw, meta = m.predict(X, p_mkt_s, calibrate=False, return_residual_meta=True, segments=segments)
        ...
        unknown = buckets.isna()
        if unknown.any():
            seg_sub = segments.reindex(unknown.index)[unknown] if isinstance(segments, pd.Series) else None
            p_raw = self.fallback_model.predict(X.loc[unknown], p_mkt_s.loc[unknown], calibrate=False, segments=seg_sub)
            out[unknown.values] = cal.transform(p_raw) if (calibrate and cal is not None) else p_raw

        for b in sorted(set(buckets.dropna().tolist())):
            mask = buckets == b
            m = self.bucket_models.get(int(b)) if b is not None else None
            m = m or self.fallback_model
            seg_sub = segments.reindex(mask.index)[mask] if isinstance(segments, pd.Series) else None
...
def train_model(
    ...
    final_model = model
    if enabled:
        min_train = int(dist_cfg.get("min_train_samples", 5000)) if isinstance(dist_cfg, dict) else 5000
        dist = pd.to_numeric(X_train.get("distance"), errors="coerce") if X_train is not None else None
        buckets_train = dist.where(dist > 0).apply(distance_bucket) if dist is not None else pd.Series([], dtype=object)

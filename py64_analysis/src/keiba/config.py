...
def load_yaml_with_extends(path: Path) -> dict:
    ...
    def _load(cur_path: Path) -> dict:
        ...
        merged: dict = {}
        if extends:
            bases = [extends] if isinstance(extends, str) else list(extends) if isinstance(extends, list) else []
            for b in bases:
                base_path = _resolve_base(cur_path, str(b))
                if not base_path.exists():
                    raise FileNotFoundError(f"config extends not found: {base_path} (from {cur_path})")

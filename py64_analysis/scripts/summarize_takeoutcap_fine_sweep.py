"""
Summarize takeout cap fine sweep across 2024/2025 eval-only rolling runs.
"""
...
def _variant_cap_value(run_dirs: list[Path]) -> Optional[float]:
    for run_dir in run_dirs:
        for w in sorted(run_dir.glob("w*/summary.json")):
            summary = _read_json(w)
            cap = summary.get("race_cost_cap_value")
            if cap is None:
                rc = summary.get("race_cost_filter") or {}
                cap = rc.get("race_cost_cap_value")
            try:
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Summarize takeout cap fine sweep (2024/2025)")
    ...
    final_report_lines.append("# Takeout cap fine sweep (2024/2025)")
    final_report_lines.append("")
    final_report_lines.append("- Split: time-based (train < valid < test), eval-only windows (design metrics are N/A).")
    final_report_lines.append("")

    gate_by_year_cap: dict[str, dict[float, dict[str, Any]]] = {"2024": {}, "2025": {}}
    cap_by_variant: dict[str, float] = {}
    ...
    for year in ["2024", "2025"]:
        ...
        for variant_name, run_list in variants.items():
            ...
            cap_val = _variant_cap_value(var_dirs)
            cap_by_variant[variant_name] = cap_val if cap_val is not None else float("nan")
            if cap_val is not None:
                gate_by_year_cap[year][cap_val] = gate
            ...
            final_report_lines.append("")
            final_report_lines.append(f"Variant: {variant_name} (cap={cap_val})")
            ...
            cap_rows_for_print.append((cap_val, gate))

        for cap_val, gate in sorted(cap_rows_for_print, key=lambda x: (float("inf") if x[0] is None else x[0])):
            cap_str = "none" if cap_val is None else f"{cap_val:.4f}"
            print(
                f"[takeoutcap {year} eval] cap={cap_str} gate_pass={gate.get('gate_pass')} | "
    ...
    caps_all = sorted(set(gate_by_year_cap.get("2024", {}).keys()) | set(gate_by_year_cap.get("2025", {}).keys()))
    pass_both_caps = []
    for cap in caps_all:
        pass_2024 = bool(gate_by_year_cap.get("2024", {}).get(cap, {}).get("gate_pass"))
        pass_2025 = bool(gate_by_year_cap.get("2025", {}).get(cap, {}).get("gate_pass"))
        ...
        if decision == "pass_both":
            pass_both_caps.append(cap)
    ...
    pass_both_variants = "none" if not pass_both_caps else ",".join(f"{c:.4f}" for c in sorted(pass_both_caps))

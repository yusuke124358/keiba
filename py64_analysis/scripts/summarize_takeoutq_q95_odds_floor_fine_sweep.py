"""Summarize q95 odds-floor fine sweep (replay-from-base)."""
...
def _variant_name(min_odds: float) -> str:
    return f"odds_floor_{str(min_odds).replace('.', 'p')}"
...
def _build_virtual_run_dir(
    base_dir: Path, min_odds: float, out_dir: Path, expected_windows: list[str]
) -> Path:
...
def main() -> int:
    ...
    ap.add_argument("--min_odds_list", required=True, help="comma-separated list, e.g., 4.0,4.2,...")
    ...
    variants = [_variant_name(v) for v in min_odds]
    ...
    for year, splits in runs.items():
        for split, mapping in splits.items():
            ...
            for v, floor in zip(variants, min_odds):
                ...
                mapping[v] = _build_virtual_run_dir(
                    mapping["base"], min_odds=floor, out_dir=out_dir, expected_windows=expected_windows
                )
                cov_var = _coverage_status(mapping[v], expected_windows)
    ...
    if not eval_df.empty and not step14_df.empty:
        ...
        eval_df.to_csv(out_ascii / "odds_floor_fine_sweep_summary.csv", index=False, encoding="utf-8")
        ...
        pd.DataFrame().to_csv(out_ascii / "odds_floor_fine_sweep_summary.csv", index=False, encoding="utf-8")
        ...
        for year in added_removed["year"].unique():
            for win in added_removed[added_removed["year"] == year]["window"].unique():
                ...
                if sub.empty:
                    continue
                ordered = sub.copy()
                ordered["min_odds"] = ordered["variant"].str.replace("odds_floor_", "").str.replace("p", ".").astype(float)
                ordered = ordered.sort_values("min_odds")
        ...
        if not eval_only.empty:
            ...
            if not stats_df.empty:
                ...
                if pass_both.empty:
                    ...
                    chosen_min_odds = chosen_variant.replace("odds_floor_", "").replace("p", ".")

    lines = ["# q95 odds floor fine sweep (eval-only)", ""]
    ...
    if chosen_variant:
        lines.append(f"- chosen_min_odds={chosen_min_odds} (reason={chosen_reason})")
    else:
        lines.append(f"- chosen_min_odds=None (reason={chosen_reason})")

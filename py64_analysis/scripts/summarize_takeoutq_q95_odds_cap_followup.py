"""Summarize q95 odds-cap follow-up (eval-only) for 2024/2025."""
...
def _detect_odds_col(cols: list[str]) -> Optional[str]:
    return _find_col(cols, ["odds_at_buy", "odds", "buy_odds"])


def _detect_stake_col(cols: list[str]) -> Optional[str]:
    return _find_col(cols, ["stake", "stake_yen", "stake_amount", "bet_amount"])


def _detect_return_col(cols: list[str]) -> Optional[str]:
    return _find_col(cols, ["return", "return_yen", "payout", "payout_yen"])


def _detect_profit_col(cols: list[str]) -> Optional[str]:
    return _find_col(cols, ["profit", "net_profit", "pnl"])
...
def _compute_window_summary(bets_path: Path, bankroll: float) -> dict[str, Any]:
    ...
    if ret_col is not None:
        ...
        profit = pd.to_numeric(df[prof_col], errors="coerce").fillna(0.0)
    else:
        profit = pd.Series([0.0] * len(df))
    ...
    if daily is None or daily.empty:
        ...
        daily = daily.reset_index().rename(columns={profit.name if hasattr(profit, "name") else "profit": "profit"})
...
def _build_virtual_run_dir(base_dir: Path, cap: float, out_dir: Path) -> Path:
    ...
    for w in sorted(base_dir.glob("w*/")):
        ...
        filtered = df[odds <= float(cap)].copy()
...
def main() -> int:
    ...
    ap.add_argument("--base-2024", required=True)
    ap.add_argument("--base-2025", required=True)
    ap.add_argument("--odds25-2024", required=True)
    ap.add_argument("--odds25-2025", required=True)
    ap.add_argument("--odds20-2024", required=True)
    ap.add_argument("--odds20-2025", required=True)
    ap.add_argument("--out-dir", required=True)
    ap.add_argument("--replay-from-base", action="store_true")
    args = ap.parse_args()
    ...
    runs = {
        2024: {
            "base": Path(args.base_2024),
            "odds25": Path(args.odds25_2024),
            "odds20": Path(args.odds20_2024),
        },
        2025: {
            "base": Path(args.base_2025),
            "odds25": Path(args.odds25_2025),
            "odds20": Path(args.odds20_2025),
        },
    }

    if args.replay_from_base:
        for year, mapping in runs.items():
            base_dir = mapping["base"]
            for variant, cap in [("odds25", 25.0), ("odds20", 20.0)]:
                virt_dir = out_dir / "virtual_runs" / str(year) / variant
                mapping[variant] = _build_virtual_run_dir(base_dir, cap=cap, out_dir=virt_dir)
    ...
    for year, mapping in runs.items():
        ...
        for variant in ("odds25", "odds20"):
            ...
            _write_debug_samples(out_dir, year, variant, mapping[variant])

        _write_debug_samples(out_dir, year, "base", mapping["base"])
    ...
    if not eval_df.empty:
        ...
        eval_df.to_csv(out_dir / "eval_gate_summary.csv", index=False, encoding="utf-8")
    ...
    if not step14_df.empty:
        step14_df.to_csv(out_dir / "walkforward_step14_summary.csv", index=False, encoding="utf-8")

    added_removed = pd.concat(binding_rows, ignore_index=True) if binding_rows else pd.DataFrame()
    if not added_removed.empty:
        added_removed.to_csv(out_dir / "added_removed_by_window.csv", index=False, encoding="utf-8")
    ...
    if not added_removed.empty:
        ...
        for year in added_removed["year"].unique():
            for win in added_removed[added_removed["year"] == year]["window"].unique():
                ...
                if hashes.get("odds25") and hashes.get("odds20") and hashes["odds25"] == hashes["odds20"]:
                    added_removed.loc[sub.index, "pass_set_discrete"] = True
                    discrete_thresholds += 1
        binding_summary = added_removed.copy()
        binding_summary.to_csv(out_dir / "binding_diagnosis.csv", index=False, encoding="utf-8")
    else:
    ...
    # final report
    lines = ["# q95 odds cap follow-up (eval-only)", ""]
    ...
    lines.append("")
    if args.replay_from_base:
        lines.append("- NOTE: odds25/odds20 are replayed from baseline bets (post-filter by odds cap); no retraining.")
        lines.append("")
    if not eval_df.empty:
    ...
    if not binding_summary.empty:
        ident = int(binding_summary["identical_passset"].sum())
        lines.append("")
